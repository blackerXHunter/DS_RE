    ...
    public class BlackAI : MonoBehaviour
    {
        ...
        public async Task AutoAttackTask(CancellationToken ct)
        {
            while (true)
            {
                ...
                if (canAttack)
                {
                    ac.model.transform.LookAt(playerTansform);
                    Attack();
                }
                else
                {
        ...
        {
            var cols = Physics.OverlapBox(this.transform.position, new Vector3(0.5f, 0.5f, distance), ac.model.transform.rotation, LayerMask.GetMask("Player"));
            if (cols.Length > 0)
            {
        ...
        private bool CheckCanAttackPlayer()
        {
            ...
            var cols = Physics.OverlapBox(this.transform.position, new Vector3(0.5f, 0.5f, 1), ac.model.transform.rotation, LayerMask.GetMask("Player"));
            if (cols.Length > 0)
            {
                return true;
            }
        ...
        public void Reset()
        {
            ...
            ac.input.Dmag = 0;
            ac.input.Dforward = Vector3.zero;
            ac.input.run = false;
            ac.input.rb = false;
            ac.input.lb = false;
        }
